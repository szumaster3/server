...
class FremennikAchievementDiary : DiaryEventHookBase(DiaryType.FREMENNIK) {
    ...
    override fun onNpcKilled(
        ...
        when (player.viewport.region!!.id) {
            ...
            11164 ->
                if (event.npc.id in CAVE_CRAWLERS) {
                    finishTask(
                        player,
                        DiaryLevel.EASY,
            ...
            10808, 10809 ->
                if (event.npc.id == NPCs.BLACK_UNICORN_133) {
                    finishTask(
                        player,
                        DiaryLevel.EASY,
            ...
            6995 ->
                if (event.npc.id == NPCs.MITHRIL_DRAGON_5363) {
                    finishTask(
                        player,
                        DiaryLevel.HARD,
            ...
            9276, 9532 ->
                if (event.npc.id in ICE_TROLLS && inEquipment(player, Items.YAK_HIDE_ARMOUR_10822, 1)) {
                    finishTask(
                        player,
                        DiaryLevel.MEDIUM,
    ...
    override fun onSummoningPointsRecharged(
        player: Player,
    ...
    override fun onInterfaceClosed(
        player: Player,
    ...
    override fun onInteracted(
        ...
        when (player.viewport.region!!.id) {
            ...
            10811 ->
                if (event.target.id == Scenery.COLLAPSED_TRAP_19233 &&
                    inBorders(
                        player,
    ...
    override fun onButtonClicked(
        ...
        when {
            inBorders(player, RELLEKKA_BLACKSMITH_AREA) -> {
                if (event.iface == Components.CRAFTING_GLASS_542 &&
                    event.buttonId == 38 &&
                    getStatLevel(
                        player,
                        Skills.CRAFTING,
