...
class HonourGuardDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        val jesterSet = anyInEquipment(player, Items.SILLY_JESTER_HAT_10836, Items.SILLY_JESTER_TOP_10837, Items.SILLY_JESTER_TIGHTS_10838, Items.SILLY_JESTER_BOOTS_10839)
        ...
        if (player.location.z == 1) {
            npc("Hail, $name!").also { stage = 8 }
            return true
        }

        if (npc.id == NPCs.HONOUR_GUARD_5515 && inBorders(player, ZoneBorders(2328, 3802, 2309, 3824))) {
            npc("Hail, $name!").also { stage = 12 }
            return true
        }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            6 -> playerl(FaceAnim.WORRIED, "No. I only use old, recycled jokes. It's better for the environment.").also { stage++ }
