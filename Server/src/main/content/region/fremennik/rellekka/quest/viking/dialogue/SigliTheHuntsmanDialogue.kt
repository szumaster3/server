...
class SigliTheHuntsmanDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            21 -> npc("We do this with a special talisman. Here, take it, it will", "let you track the Draugen while it's invisible, and when", "you defeat it will absorb its essence.").also {
                stage++
                player?.inventory?.add(Item(3696))
            }
            ...
            23 -> npc("Take care of the talisman, and see me when you have", "completed this task.").also { stage = 1000 }
            ...
            150 -> playerl(FaceAnim.ASKING, "I don't suppose you have any idea where I could find a map to unspoiled hunting grounds, do you?").also { stage++ }
            151 -> npcl(FaceAnim.HAPPY, "Well, of course I do. I wouldn't be much of a huntsman if I didn't know where to find my prey now, would I outerlander?").also { stage++ }
            152 -> playerl(FaceAnim.ASKING, "No, I guess not. So can I have it?").also { stage++ }
            153 -> npcl(FaceAnim.ANNOYED, "Directions to my hunting grounds could mean the end of my livelihood. The only way I would be prepared to give them up would be...").also { stage++ }
            154 -> playerl(FaceAnim.THINKING, "What? Power? Money? Women? Wine?").also { stage++ }
            155 -> npcl(FaceAnim.HAPPY, "...a new string for my hunting bow. Not just any bowstring; I need a custom bowstring, balanced for my bow precisely to keep my hunt competitive.").also { stage++ }
            156 -> npcl(FaceAnim.HAPPY, "Only in this way would I allow the knowledge of my hunting grounds to be passed on to strangers.").also { stage++ }
            157 -> playerl(FaceAnim.THINKING, "So where would I get that?").also { stage++ }
            158 -> npcl(FaceAnim.HAPPY, "I have no idea. But then again, I'm happy with my old bowstring and being the only person who knows where my hunting ground is.").also {
                player.incrementAttribute(GameAttributes.QUEST_VIKING_SIGMUND_PROGRESS, 1)
                stage = 1000
            }
            160 -> playerl(FaceAnim.ASKING, "I don't suppose you have any idea where I could find a finely balanced custom bowstring, do you?").also { stage++ }
            161 -> npcl(FaceAnim.ANNOYED, "If I knew I would not have asked you to go and get me one, now would I?").also { stage = 1000 }
            165 -> playerl(FaceAnim.HAPPY, "Here. I have your bowstring. Give me your map to the hunting grounds.").also {
                removeItem(player, Items.CUSTOM_BOW_STRING_3702)
                addItemOrDrop(player, Items.TRACKING_MAP_3701, 1)
                stage++
            }
            166 -> npcl(FaceAnim.HAPPY, "Well met, outerlander. I see some hunting potential within you. Here, take my map, I was getting too dependent on it for my skill anyway.").also { stage = 1000 }
            170 -> npcl(FaceAnim.HAPPY, "No outerlander... it is hard to explain. That map makes my role as huntsman too easy. I fear my skills are becoming dulled.").also { stage++ }
            171 -> npcl(FaceAnim.HAPPY, "Now I must track my prey once more. To begin again from scratch... I feel this may keep me sharp.").also { stage = 1000 }
            175 -> npcl(FaceAnim.ANNOYED, "Not the one I want, outerlander.").also { stage = 1000 }
            180 -> npcl(FaceAnim.HAPPY, "Hello again to you ${FremennikTrials.getFremennikName(player)}. How goes the hunt?").also { stage++ }
            181 -> playerl(FaceAnim.ASKING, "They hunt? I've already done your task! That's why I'm a Fremennik now!").also { stage++ }
            182 -> npcl(FaceAnim.HAPPY, "You think so do you? The Draugen is always out there, just as I and my hunters are always on its trail to keep it at bay. The hunt will never end until I do.").also { stage = 1000 }
            1000 -> end()
        }
        return true
    }
