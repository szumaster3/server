package content.region.karamja.tzhaar_city.quest.dillo.dialogue
...
class TzHaarHurFrokDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            1 -> when (buttonId) {
                1 -> player(FaceAnim.ASKING, "Can you treat these oak planks for me? I need them", "to be fire-resistant before I can start repairing the", "tunnels.").also { stage++ }
            ...
            4 -> when (buttonId) {
                1 -> {
                    ...
                    val amount = amountInInventory(player, Items.OAK_PLANK_8778)
                    if (amount <= 0) {
