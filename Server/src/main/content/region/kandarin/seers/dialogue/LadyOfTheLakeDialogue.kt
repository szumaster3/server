...
class LadyOfTheLakeDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        val canSeekSword = getQuestStage(player!!, Quests.MERLINS_CRYSTAL) >= 40
        val missingSword = !player!!.hasItem(Item(Items.EXCALIBUR_35, 1))
        ...
        if (!isQuestComplete(player, Quests.MERLINS_CRYSTAL)) {
            ...
            when (stage) {
                0 -> {
                    if (player!!.achievementDiaryManager.getDiary(DiaryType.SEERS_VILLAGE)!!.isComplete(2) && DiaryManager(player).headband == 2 && player!!.equipment.contains(Items.EXCALIBUR_35, 1)) {
                ...
                117 -> player("Thanks!").also { stage = 58 }
                1 -> showTopics(
                    Topic(FaceAnim.NEUTRAL, "Who are you?", 2),
                    Topic(FaceAnim.NEUTRAL, "Good day.", 5),
                ...
                2 -> npcl(FaceAnim.NEUTRAL, "I am the Lady of the Lake.").also { stage = END_DIALOGUE }
                5 -> npcl(FaceAnim.NEUTRAL, "Good day to you ${if (player!!.isMale) "sir" else "madam"}.").also { stage = 58 }
                ...
                57 -> interpreter!!.sendDialogue("The lady of the Lake hands you Excalibur.").also { stage++ }
                58 -> end()
            }
        }
        return true
