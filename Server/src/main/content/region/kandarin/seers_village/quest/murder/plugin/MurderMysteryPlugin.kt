...
class MurderMysteryPlugin : InteractionListener {
    companion object {
        val SILVER_ITEMS = intArrayOf(
        ...
        val MURDER_MYSTERY_SCENERY_IDS = intArrayOf(
        ...
        val FINGERPRINTS_ITEM_IDS = intArrayOf(
    ...
    override fun defineListeners() {
        ...
        /*
         * Handles taking the criminal's dagger from the ground.
         */

        on(Items.CRIMINALS_DAGGER_1813, IntType.GROUND_ITEM, "take") { player, groundItem ->
        ...
        /*
         * Handles taking the pungent pot from the ground.
         */

        on(Items.PUNGENT_POT_1812, GROUND_ITEM, "take") { player, groundItem ->
            when (getQuestStage(player, Quests.MURDER_MYSTERY)) {
                1 -> {
        ...
        /*
         * Handles investigating sturdy wooden gates.
         */
        ...
        on(intArrayOf(Scenery.STURDY_WOODEN_GATE_2664, Scenery.STURDY_WOODEN_GATE_2665), SCENERY, "investigate") { player, _ ->
            when (getQuestStage(player, Quests.MURDER_MYSTERY)) {
                1 -> {
                    ...
                    setAttribute(player, MurderMystery.attributeNoiseClue, true)
        ...
        /*
         * Handles investigating the smashed window for threads.
         */
        ...
        on(Scenery.SMASHED_WINDOW_26110, SCENERY, "investigate") { player, _ ->
            when (getQuestStage(player, Quests.MURDER_MYSTERY)) {
                1 -> {
                    ...
                    } else if (getAttribute(player, MurderMystery.attributeTakenThread, false)) {
                        if (hasSpaceFor(player, Item(Items.CRIMINALS_THREAD_1808))) {
                            when (getAttribute(player, MurderMystery.attributeRandomMurderer, 0)) {
                            ...
                            sendMessage(
                                player,
                                "Lucky for you there's some thread left. You should be less careless in the future."
                            )
                        }
                    } else {
                        if (hasSpaceFor(player, Item(Items.CRIMINALS_THREAD_1808))) {
                            ...
                            setAttribute(player, MurderMystery.attributeTakenThread, true)
                        }
                    }
                    return@on true
                }
        ...
        /*
         * Prevents breaking the smashed window to preserve evidence.
         */
        ...
        /*
         * Handles investigating the sacks (flypaper interaction).
         */
        ...
        /*
         * Handles searching barrels for silver items.
         */

        on(MURDER_MYSTERY_SCENERY_IDS, SCENERY, "search") { player, node ->
            when (getQuestStage(player, Quests.MURDER_MYSTERY)) {
                1 -> {
                    when (node.id) {
                        Scenery.ANNA_S_BARREL_2656 -> {
        ...
        /*
         * Handles investigating general scenery for poison clues.
         */

        on(SCENERY_IDS, SCENERY, "investigate") { player, node ->
            when (node.id) {
                Scenery.SINCLAIR_FAMILY_COMPOST_HEAP_26120 -> {
                    if (!getAttribute(player, MurderMystery.attributeAskPoisonAnna, false)) {
                        sendDialogue(player, "It's a heap of compost.")
                    } else if (getAttribute(player, MurderMystery.attributeRandomMurderer, 0) == 0) {
                        sendDialogue(
                            player,
                        ...
                        setAttribute(player, MurderMystery.attributePoisonClue, 2)
                    } else {
                ...
                Scenery.SINCLAIR_FAMILY_BEEHIVE_26121 -> {
                    if (!getAttribute(player, MurderMystery.attributeAskPoisonBob, false)) {
                        sendDialogue(player, "It's a very old beehive.")
                    } else if (getAttribute(player, MurderMystery.attributeRandomMurderer, 0) == 1) {
                ...
                Scenery.SINCLAIR_MANSION_DRAIN_2843 -> {
                    if (!getAttribute(player, MurderMystery.attributeAskPoisonCarol, false)) {
                        sendDialogue(player, "It's the drains from the kitchen.")
                    } else if (getAttribute(player, MurderMystery.attributeRandomMurderer, 0) == 2) {
                ...
                Scenery.SPIDERS_NEST_26109 -> {
                    if (!getAttribute(player, MurderMystery.attributeAskPoisonDavid, false)) {
                        sendDialogue(player, "It looks like a spiders' nest of some kind...")
                    } else if (getAttribute(player, MurderMystery.attributeRandomMurderer, 0) == 3) {
                ...
                Scenery.SINCLAIR_FAMILY_FOUNTAIN_2654 -> {
                    if (!getAttribute(player, MurderMystery.attributeAskPoisonElizabeth, false)) {
                        sendDialogue(player, "A fountain with large numbers of insects around the base.")
                    } else if (getAttribute(player, MurderMystery.attributeRandomMurderer, 0) == 4) {
                ...
                Scenery.SINCLAIR_FAMILY_CREST_2655 -> {
                    if (!getAttribute(player, MurderMystery.attributeAskPoisonFrank, false)) {
                        sendDialogue(player, "The Sinclair Family Crest is hung up here.")
                    } else if (getAttribute(player, MurderMystery.attributeRandomMurderer, 0) == 5) {
        ...
        /*
         * Handles using flour on silver items to prepare evidence.
         */

        onUseWith(ITEM, SILVER_ITEMS, Items.POT_OF_FLOUR_1933) { player, used, _ ->
            when (used.id) {
                Items.CRIMINALS_DAGGER_1813 -> {
        ...
        /*
         * Handles using flypaper on dusted items to take fingerprint impressions.
         */

        onUseWith(ITEM, DUSTED_ITEMS, Items.FLYPAPER_1811) { player, used, _ ->
        ...
        /*
         * Handles comparing taken fingerprint with unknown print to identify the suspect.
         */

        onUseWith(ITEM, FINGERPRINTS_ITEM_IDS, Items.UNKNOWN_PRINT_1822) { player, used, with ->
            ...
            if (used.id == getAttribute(player, MurderMystery.attributeRandomMurderer, 0) + Items.ANNAS_PRINT_1816) {
        ...
        /*
         * Prevents using evidence items to store flour in the barrel.
         */
