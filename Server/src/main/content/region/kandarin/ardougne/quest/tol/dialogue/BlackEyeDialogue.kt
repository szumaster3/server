    ...
    override fun handle(componentID: Int, buttonID: Int): Boolean {
        when (getQuestStage(player!!, Quests.TOWER_OF_LIFE)) {
            0 -> when (stage) {
                ...
                1 -> npcl(FaceAnim.FRIENDLY, "Ah, we be building this 'ere tower. Lookin' good ain't it?").also { stage++ }
                2 -> playerl(FaceAnim.FRIENDLY, "It does look pretty impressive, but what are you doing hanging around?").also { stage++ }
            ...
            2 -> when (stage) {
                ...
                2 -> playerl(FaceAnim.FRIENDLY, "I was just wondering if you might be able to help me out with one?").also { stage++ }
                3 -> npcl(FaceAnim.FRIENDLY, "Always glad to help a budding builder.").also { stage++ }
                4 -> npcl(FaceAnim.FRIENDLY, "But first, prove your Construction knowledge by answering some questions.").also { stage++ }
                ...
                8 -> npcl(FaceAnim.FRIENDLY, "One? How do you suppose to use a single nail?").also { stage = END_DIALOGUE }
                9 -> npcl(FaceAnim.FRIENDLY, "Nope. Keep guessing!").also { stage = END_DIALOGUE }
                10 -> npcl(FaceAnim.FRIENDLY, "Bingo! Okay, now what takes 3 planks, 3 cloths, and 3 nails to make, and help remove light from a room?").also { stage++ }
                ...
                13 -> npcl(FaceAnim.FRIENDLY, "Of course, because that's what everyone needs - a rug up at their window.").also { stage = END_DIALOGUE }
                14 -> npcl(FaceAnim.FRIENDLY, "Those curtains are not going to be in any way opulent, matey.").also { stage = END_DIALOGUE }
                15 -> npcl(FaceAnim.FRIENDLY, "Nice one. Last question: I like fish and I want to put some in my garden, but I need a special water feature. What materials would I require?").also { stage++ }
                ...
                21 -> npcl(FaceAnim.FRIENDLY, "You seem to know your stuff. I got a spare helmet from a builder that died on... I mean, err, had to leave the job for greener pastures.").also { stage++ }
                ...
                25 -> npcl(FaceAnim.FRIENDLY, "Erm, no, that's just, erm, paint. Yes, paint! That's what it is!").also { stage++ }
