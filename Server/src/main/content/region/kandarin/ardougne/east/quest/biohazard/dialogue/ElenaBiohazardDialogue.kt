        ...
        when {
            (questStage == 0) -> {
                when (stage) {
                    ...
                    2 -> npc(FaceAnim.STRUGGLE, "It's just a shame the mourners confiscated my", "equipment.").also { stage++ }
                    3 -> player(FaceAnim.HALF_ASKING, "What did they take?").also { stage++ }
                    4 -> npc(FaceAnim.HALF_GUILTY, "My distillator, I can't test any plague samples without it.", "They're holding it in the mourner quarters in West", "Ardougne.").also { stage++ }

                    5 -> npc(FaceAnim.HALF_GUILTY, "I must somehow retrieve that distillator if I am to find", "a cure for this awful affliction.").also { stage++ }
                    ...
                    9 -> npc(FaceAnim.SAD, "I was hoping you would say that. Unfortunately they", "discovered the tunnel and filled it in. We need another", "way over the wall.").also { stage++ }

                    10 -> player(FaceAnim.HALF_ASKING, "Any ideas?").also { stage++ }
                    11 -> npc(FaceAnim.FRIENDLY, "My father's friend Jerico is in communication with", "West Ardougne. He might be able to help us, he lives", "next to the chapel.").also { stage++ }
            ...
            (questStage == 4) -> {
                when (stage) {
                    ...
                    1 -> npc(FaceAnim.HAPPY, "Yes, I saw. Quickly meet with Jerico's friends", "and cross the wall before the pigeons fly off.").also { stage++ }
            ...
            (questStage in 10..15) -> {
                when (stage) {
                    0 -> if (getAttribute(player!!, GameAttributes.ELENA_REPLACE, false)) {
                    ...
                    4 -> end()
                    5 -> {
                        npc(FaceAnim.HAPPY, "You have? That's great! Now can you pass me those", "reaction agents please?")
                    ...
                    9 -> {
                        npc(FaceAnim.STRUGGLE, "I don't understand... the touch paper hasn't changed", "colour at all...")
                        sendMessage(player!!, "Elena puts the agents through the distillator.")
                        stage++
                    }
                    10 -> npc(FaceAnim.THINKING, "You'll need to go and see my old mentor Guidor. He", "lives in Varrock. Take these vials and this sample to", "him.").also { stage++ }
                    11 -> npc(FaceAnim.NEUTRAL, "But first you'll need some more touch paper. Go and", "see the chemist in Rimmington.").also { stage++ }
                    12 -> npc(FaceAnim.NEUTRAL, "Just don't get into any fights, and be careful who", "you speak to.").also { stage++ }
                    13 -> npc(FaceAnim.NEUTRAL, "Those vials are fragile, and plague carriers don't", "tend to be too popular.").also { stage++ }
                    14 -> {
                        end()
                        if (removeItem(player!!, Items.DISTILLATOR_420)) {
                            REQUIRED_ITEMS.forEach { addItemOrDrop(player!!, it) }
                    ...
                    100 -> npc(FaceAnim.HALF_GUILTY, "Yes... I have feelings for you too...", "Now get to work!").also { stage = END_DIALOGUE }
                    200 -> {
                        if (REQUIRED_ITEMS.any { !inInventory(player!!, it) }) {
                            if(freeSlots(player!!) < REQUIRED_ITEMS.size){
                                ...
                                REQUIRED_ITEMS.forEach { if (!inInventory(player!!, it)) addItem(player!!, it) }
                                stage++
                            }
                        } else {
                            npcl(FaceAnim.HALF_THINKING, "Are you sure? Looks like you have everything to me.").also { stage = END_DIALOGUE }
                        }
                    }
                    201 -> npc(FaceAnim.HALF_THINKING, "Ok, so that's the colourless ethenea...", "Some highly toxic sulphuric broline...", "And some bog-standard liquid honey...").also { stage++ }
                    202 -> playerl(FaceAnim.FRIENDLY, "Great. I'll be on my way.").also { stage = END_DIALOGUE }

                    300 -> npcl(FaceAnim.HALF_THINKING, "Go to Rimmington and get some touch paper from the chemist. Use his errand boys to smuggle the vials into Varrock, then go to Varrock and take the sample to Guidor, my old mentor.").also { stage++ }
            ...
            (questStage == 100) -> {
                when (stage) {
                    ...
                    3 -> npc(FaceAnim.HAPPY, "Not bad, let me know when you hear from", "King Lathas again.").also { stage++ }
