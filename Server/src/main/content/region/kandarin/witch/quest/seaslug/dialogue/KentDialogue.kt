...
class KentDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            3 -> npc(FaceAnim.NEUTRAL, "Five days ago we pulled in a huge catch. As well as fish", "we caught small slug like creatures, hundreds of them.").also { stage++ }
            4 -> npc(FaceAnim.NEUTRAL, "That's when the fishermen began to act strange.").also { stage++ }
            5 -> npc(FaceAnim.NEUTRAL, "It was the sea slugs, they attack themselves to your", "body and somehow take over the mind of the carrier.").also { stage++ }
            6 -> npc(FaceAnim.NEUTRAL, "I told Kennith to hide until I returned but I was", "washed up here.").also { stage++ }
            7 -> npc(FaceAnim.NEUTRAL, "Please go back and get my boy, you can send help for", "me later.").also { stage++ }
    ...
    internal class KentDialogueFile : DialogueFile() {
        override fun handle(
            ...
            when (stage) {
                0 -> npc(FaceAnim.CALM_TALK, "A few more minutes and that thing would have full", "control of your body.").also { stage++ }
                1 -> player(FaceAnim.SCARED,"Yuck! Thanks Kent.").also { stage = END_DIALOGUE }
            }
        }
    }
}
