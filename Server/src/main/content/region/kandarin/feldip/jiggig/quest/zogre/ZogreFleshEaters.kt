package content.region.kandarin.feldip.jiggig.quest.zogre
...
class ZogreFleshEaters : Quest(Quests.ZOGRE_FLESH_EATERS, 40, 39, 1, Vars.VARBIT_QUEST_ZORGE_FLESH_EATERS_PROGRESS_487, 0, 1, 13) {
    ...
    override fun drawJournal(player: Player, stage: Int) {
        ...
        if (stage == 0) {
            ...
            line(player, if (isQuestComplete(player, Quests.JUNGLE_POTION,)) { "---${Quests.JUNGLE_POTION}./--" } else { "!!${Quests.JUNGLE_POTION}.??" }, line++,)
            line(player, if (isQuestComplete(player, Quests.BIG_CHOMPY_BIRD_HUNTING,)) { "---${Quests.BIG_CHOMPY_BIRD_HUNTING}./--" } else { "!!${Quests.BIG_CHOMPY_BIRD_HUNTING}.??" }, line++,)
        ...
        if (getVarbit(player, Vars.VARBIT_QUEST_ZORGE_FLESH_EATERS_PROGRESS_487) >= 1) {
            line(player, "I talked to !!Grish?? in the !!Jiggig?? area which is swarming with", line++, getVarbit(player, Vars.VARBIT_QUEST_ZOGRE_COFFIN_TRANSFORM_488) >= 3)
            line(player, "Zombie Ogres (Zogres) These disgusting creatures carry", line++, getVarbit(player, Vars.VARBIT_QUEST_ZOGRE_COFFIN_TRANSFORM_488) >= 3)
            line(player, "disease and are quite dangerous so the Ogres weren't", line++, getVarbit(player, Vars.VARBIT_QUEST_ZOGRE_COFFIN_TRANSFORM_488) >= 3)
            line(player, "too keen to try and sort them out", line++, getVarbit(player, Vars.VARBIT_QUEST_ZOGRE_COFFIN_TRANSFORM_488) >= 3)
            line++
        }

        if (getVarbit(player, Vars.VARBIT_QUEST_ZOGRE_COFFIN_TRANSFORM_488) == 3) {
            line(player, "I talked to an ogre called !!Grish?? who asked me to", line++, getVarbit(player, Vars.VARBIT_QUEST_ZORGE_FLESH_EATERS_PROGRESS_487) >= 6)
            line(player, "look into the problem. After !!some searching around in a??", line++, getVarbit(player, Vars.VARBIT_QUEST_ZORGE_FLESH_EATERS_PROGRESS_487) >= 6)
            line(player, "!!tomb??, I found some clues which pointed me to the !!human??", line++, getVarbit(player, Vars.VARBIT_QUEST_ZORGE_FLESH_EATERS_PROGRESS_487) >= 6)
            line(player, "!!habitation of Yannile??.", line++, getVarbit(player, Vars.VARBIT_QUEST_ZORGE_FLESH_EATERS_PROGRESS_487) >= 6)
            line++
        }

        if (getVarbit(player, Vars.VARBIT_QUEST_ZORGE_FLESH_EATERS_PROGRESS_487) >= 6) {
            line(player, "With the help of Zavistic Rarve, the grand secretary of the", line++, getVarbit(player, Vars.VARBIT_QUEST_ZORGE_FLESH_EATERS_PROGRESS_487) >= 8)
            line(player, "Wizards guild I was able to piece the clues together and", line++, getVarbit(player, Vars.VARBIT_QUEST_ZORGE_FLESH_EATERS_PROGRESS_487) >= 8)
            line(player, "discover that a Wizard named 'Sithik Ints' was responsible", line++, getVarbit(player, Vars.VARBIT_QUEST_ZORGE_FLESH_EATERS_PROGRESS_487) >= 8)
            line++
        }
        ...
        if (getAttribute(player, ZogreUtils.TALK_WITH_SITHIK_OGRE_DONE, false) || getVarbit(player, Vars.VARBIT_QUEST_ZORGE_FLESH_EATERS_PROGRESS_487) >= 12) {
            line(player, "Sithik Ints also told me how to make Brutal arrows which are", line++, getVarbit(player, Vars.VARBIT_QUEST_ZORGE_FLESH_EATERS_PROGRESS_487) == 13)
            line(player, "more effective against Zogres, and he also told me how to", line++, getVarbit(player, Vars.VARBIT_QUEST_ZORGE_FLESH_EATERS_PROGRESS_487) == 13)
            line(player, "make a disease balm", line++, getVarbit(player, Vars.VARBIT_QUEST_ZORGE_FLESH_EATERS_PROGRESS_487) == 13)
            line++
        }
