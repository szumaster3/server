...
class DoricDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        when (quesStage) {
            0 -> npcl(FaceAnim.OLD_NORMAL, "Hello traveller, what brings you to my humble smithy?").also { stage = 0 }
            in 1..99 -> openDialogue(player, DoricDialogue(30), npc)
            else -> npcl(FaceAnim.OLD_HAPPY, "Hello traveller, how is your metalworking coming along?").also { stage = 60 }
        }
        return true
    }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            30 -> if (getAttribute(player, "pre-dq:doric-angy-count", 0) == 10) {
                end()
            ...
            62 -> {
                if (!getAttribute(player, "pre-dq:said-hi", false)) end()
                else playerl(FaceAnim.FRIENDLY, "By the way, I told Nurmof you said hello.").also { stage++ }
            }
            63 -> {
