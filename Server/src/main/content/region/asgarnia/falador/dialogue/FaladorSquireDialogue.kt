...
class FaladorSquireDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        npc = args[0] as NPC
        sendOptions(player, "What do you want to do?", "Chat", "Talk about the Falador Achievement Diary")
        stage = -1
        return true
    }
        ...
        if ((stage == -1 && buttonId == 2) || Diary.canReplaceReward(player, DiaryType.FALADOR, 2)) {
            ...
            when (getQuestStage(player, Quests.THE_KNIGHTS_SWORD)) {
                ...
                20, 10 -> when (stage) {
                    ...
                    1 -> npc("Please try and find them quickly... I am scared Sir", "Vyvin will find out!").also {
                        stage = END_DIALOGUE
                    }
                }
                ...
                0 -> when (stage) {
                    ...
                    100 -> npc("Well now, if I knew THAT it wouldn't be lost, now", "would it?").also {
                        stage = END_DIALOGUE
                    }
                    ...
                    123 -> options("So would these dwarves make another one?", "Well I hope you find it soon.").also {
                        stage = 134
                    }
