...
class FortressGuardDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        npc = args[0] as NPC
        if (args.size == 2) {
            npc(FaceAnim.ANGRY, "Hey! You can't come in here! This is a high security", "military installation!").also { stage = 40 }
            return true
        }
        if (args.size == 3) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            1 -> npc(FaceAnim.NEUTRAL, "Well, this is the guards' entrance. I might be new here", "but I can tell you're not a guard.").also { stage++ }
            ...
            20 -> npc(FaceAnim.NEUTRAL, "Well you can see me wearing it. It's an iron chainbody", "and a medium bronze helm.").also { stage++ }
            21 -> player(FaceAnim.HALF_ASKING, "Hmmm... I wonder if I can make that or get some in", "the local towns...").also { stage++ }
