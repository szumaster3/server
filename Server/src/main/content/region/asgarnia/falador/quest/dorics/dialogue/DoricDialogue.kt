package content.region.asgarnia.falador.quest.dorics.dialogue
...
class DoricDialogue(private val questStage: Int) : DialogueFile() {
    ...
    private fun handleQuestStartDialogue(player: Player?, isWhetstone: Boolean) {
        ...
        when (stage) {
            ...
            100 -> npcl(FaceAnim.OLD_NORMAL, "That is your choice. Nice to meet you anyway.").also { stage = END_DIALOGUE }
        }
    }
    ...
    private fun handleGiveDoricItemsDialogue(player: Player?) {
        ...
        when (stage) {
            ...
            52 -> npcl(FaceAnim.OLD_NORMAL, "You'll be able to find all those ores in the rocks just inside the Dwarven Mine. Head east from here and you'll find the entrance in the side of Ice Mountain.").also { stage = END_DIALOGUE }
        }
    }
}
