...
class MiningInstructorDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (getAttribute(player, GameAttributes.TUTORIAL_STAGE, 0)) {
            ...
            34, 35 -> when (stage) {
                0 -> sendNPCDialogue(player, npc.id, "Absolutely right, ${player.username}. These two ore types can be smelted together to make bronze.").also {
                    stage++
                }

                1 -> sendNPCDialogue(player, npc.id, "So now you know what ore is in the rocks over there, why don't you have a go at mining some tin and copper? Here, you'll need this to start with.").also {
                    stage++
                }
            ...
            40, 41 -> when (stage) {
                0 -> sendNPCDialogue(player, npc.id, "Okay, I'll show you how to make a dagger out of it. You'll be needing this..").also {
                    stage++
                }
