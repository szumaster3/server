...
class SurvivalExpertDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        when (tutStage) {
            4 -> npc("Hello there, newcomer. My name is Brynna. My job is", "to teach you a few survival tips and tricks. First off", "we're going to start with the most basic survival skill of", "all: making a fire.")
            11 -> npc("Well done! Next we need to get some food in our", "bellies. We'll need something to cook. There are shrimp", "in the pond there, so let's catch and cook some.")
            5, 14, 15 -> {
        ...
        when (player?.getAttribute(GameAttributes.TUTORIAL_STAGE, 0)) {
            ...
            16 -> when (stage) {
                0 -> when (buttonId) {
                    ...
                    3 -> npcl(FaceAnim.HAPPY, "Whenever you see bubbles in the water, there's probably some good fishing to be had there!").also { stage = 4 }
