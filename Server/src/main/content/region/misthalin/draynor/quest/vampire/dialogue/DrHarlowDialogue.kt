...
class DrHarlowDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            0 -> if (getQuestStage(player, Quests.VAMPIRE_SLAYER) == 10) {
            ...
            7 -> npc(FaceAnim.DRUNK, "Buy me a beer... then I'll teash you what you need to", "know...").also { stage++ }
            ...
            30 -> playerl(FaceAnim.HAPPY, "I successfully slayed the vampire! Morgan's village is now free of threat and all is well.").also { stage++ }
            31 -> npcl(FaceAnim.DRUNK, "Well, that calls for a drink to celebrate! How 'bout yoush buy me one?").also { stage = END_DIALOGUE }
        }
        return true
    }
