    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            15 -> when (buttonId) {
                ...
                2 -> player("Can you make me some?").also { stage = 30 }
            }
    ...
    companion object {
        ...
        private fun handleExchange(player: Player, npc: NPC, bark: SplitBark, amount: Int) {
            ...
            if (player.inventory.getAmount(COINS) < totalCost && player.inventory.getAmount(BARK) < totalBarkRequired) {
                sendNPCDialogue(player, NPCs.WIZARD_1263, "You need at least $totalBarkRequired pieces of bark and $totalCost coins to make this.")
                return
            }
            if (player.inventory.getAmount(BARK) < totalBarkRequired) {
                sendNPCDialogue(player, NPCs.WIZARD_1263, "You need at least $totalBarkRequired pieces of bark for this.")
                return
            }
            ...
            if (player.inventory.remove(Item(BARK, totalBarkRequired)) && player.inventory.remove(Item(COINS, totalCost))) {
