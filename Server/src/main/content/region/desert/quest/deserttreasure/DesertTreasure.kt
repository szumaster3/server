 ...
 *
 * **Shadow Varbit**
 * - Controls the state switch for indivisible NPCs in [General Shadow][content.region.fremennik.rellekka.miniquest.shadow_maj.GeneralShadow] and [Curse of Zaros][content.region.kandarin.miniquest.secret_ghost.ViggoraDialogue]
 *
 * ```kotlin
...
class DesertTreasure : Quest(Quests.DESERT_TREASURE, 45, 44, 3, 440, 0, 1, 15) {
    companion object {
        ...
        fun hasRequirements(player: Player): Boolean = arrayOf(
    ...
    override fun drawJournal(player: Player, stage: Int) {
        ...
        if (!started) {
            ...
            line(player, "${Quests.THE_DIG_SITE} Quest", line++, isQuestComplete(player, Quests.THE_DIG_SITE))
