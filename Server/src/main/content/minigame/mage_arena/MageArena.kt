...
import content.minigame.mage_arena.npc.KolodionNPC
...
class MageArena : MapZone("mage arena", true, ZoneRestriction.RANDOM_EVENTS), Plugin<Any> {
            ...
            if (!logout) {
                Pulser.submit(object : Pulse(1, e) {
                    override fun pulse(): Boolean {
                        if (!p.zoneMonitor.isInZone("mage arena") && hasSession(p)) {
                            getSession(p).close()
                        }
                        return true
                    }
    ...
    override fun continueAttack(e: Entity, target: Node, style: CombatStyle, message: Boolean): Boolean {
        ...
        if (target is Player && hasSession(target) && e !is KolodionNPC) return false
    ...
    /**
     * Checks if the player has a active session.
     */
