...
public final class PestControlSession {
	...
	private void addPlayer(Player p, String portalHealth) {
		...
		p.getProperties().setTeleportLocation(l.transform(32 + r.nextInt(4), 49 + r.nextInt(6), 0));
                setVarp(p, 1147, 0);
		p.getDialogueInterpreter().sendDialogues(3781, FaceAnim.FURIOUS, "You must defend the Void Knight while the portals are", "unsummoned. The ritual takes twenty minutes though,", "so you can help out by destroying them yourselves!", "Now GO GO GO!");
	}
