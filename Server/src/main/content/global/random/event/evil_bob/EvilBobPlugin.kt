...
class EvilBobPlugin :
    ...
    override fun defineListeners() {
        on(EvilBobUtils.evilBob, IntType.NPC, "talk-to") { player, node ->
            openDialogue(player, content.global.random.event.evil_bob.EvilBobDialogue(), node.asNpc())
            return@on true
        }
        ...
        onUseWith(IntType.NPC, EvilBobUtils.fishlikeThings, EvilBobUtils.evilBob) { player, _, _ ->
            openDialogue(player, content.global.random.event.evil_bob.EvilBobDialogue(), NPCs.EVIL_BOB_2479)
            return@onUseWith true
        }
