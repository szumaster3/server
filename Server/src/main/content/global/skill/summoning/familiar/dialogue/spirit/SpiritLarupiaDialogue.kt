...
class SpiritLarupiaDialogue : Dialogue {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        if (branch == -1) {
            ...
            when (branch) {
                ...
                2 -> npcl(FaceAnim.CHILD_NORMAL, "What are we doing today, master?")
                3 -> npcl(FaceAnim.CHILD_NORMAL, "Master, do you ever worry that I might eat you?")
            }
        }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (branch) {
            0 -> {
                when (stage) {
                    0 -> { npcl(FaceAnim.CHILD_NORMAL, "What is your wish master?"); stage++ }
                    1 -> { playerl(FaceAnim.FRIENDLY, "Have you ever thought about doing something other than hunting and serving me?"); stage++ }
                    2 -> { npcl(FaceAnim.CHILD_NORMAL, "You mean, like stand-up comedy, master?"); stage++ }
                    3 -> { playerl(FaceAnim.FRIENDLY, "Umm...yes, like that."); stage++ }
                    4 -> { npcl(FaceAnim.CHILD_NORMAL, "No, master."); stage = END_DIALOGUE }
                }
            }
            1 -> {
                when (stage) {
                    0 -> { npcl(FaceAnim.CHILD_NORMAL, "'Friend', master? I do not understand this word."); stage++ }
                    1 -> { playerl(FaceAnim.FRIENDLY, "Friends are people, or animals, who like one another. I think we are friends."); stage++ }
                    2 -> { npcl(FaceAnim.CHILD_NORMAL, "Ah, I think I understand friends, master."); stage++ }
                    3 -> { playerl(FaceAnim.FRIENDLY, "Great!"); stage++ }
                    4 -> { npcl(FaceAnim.CHILD_NORMAL, "A friend is someone who looks tasty, but you don't eat."); stage++ }
            ...
            2 -> {
                when (stage) {
                    ...
                    1 -> { npcl(FaceAnim.CHILD_NORMAL, "I desire only to hunt and to serve my master."); stage++ }
            ...
            3 -> {
                when (stage) {
                    ...
                    1 -> { npcl(FaceAnim.CHILD_NORMAL, "That is good, master."); stage++ }
                    2 -> { playerl(FaceAnim.FRIENDLY, "Should I?"); stage++ }
                    3 -> { npcl(FaceAnim.CHILD_NORMAL, "Of course not, master."); stage++ }
