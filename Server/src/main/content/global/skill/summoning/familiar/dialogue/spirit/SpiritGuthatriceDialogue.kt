...
/**
 * Represents the Spirit Guthatrice familiar dialogue.
 */
@Initializable
        ...
        when (branch) {
            0 -> {
                when (stage) {
                    ...
                    11 -> { npcl(FaceAnim.HALF_ASKING, "Is this what you do for fun?"); stage++ }
                    12 -> { playerl(FaceAnim.FRIENDLY, "Sometimes. Why, what do you do for fun?"); stage++ }
                    13 -> { npcl(FaceAnim.FRIENDLY, "I find things and glare at them until they die!"); stage++ }
                    14 -> { playerl(FaceAnim.FRIENDLY, "Well...everyone needs a hobby, I suppose."); stage = END_DIALOGUE }
                }
            }
            1 -> {
                when (stage) {
                    15 -> { playerl(FaceAnim.FRIENDLY, "You know, I think I might train as a hypnotist."); stage++ }
                    16 -> { playerl(FaceAnim.HALF_ASKING, "Isn't that an odd profession for a cockatrice?"); stage++ }
                    17 -> { npcl(FaceAnim.FRIENDLY, "Not at all! I've already been practicing!"); stage++ }
                    18 -> { playerl(FaceAnim.FRIENDLY, "Oh, really? How is that going?"); stage++ }
                    19 -> { npcl(FaceAnim.FRIENDLY, "Not good. I tell them to look in my eyes and that they are feeling sleepy."); stage++ }
                    20 -> { playerl(FaceAnim.FRIENDLY, "I think I can see where this is headed."); stage++ }
                    21 -> { npcl(FaceAnim.FRIENDLY, "And then they just lie there and stop moving."); stage++ }
                    22 -> { playerl(FaceAnim.FRIENDLY, "I hate being right sometimes."); stage = END_DIALOGUE }
                }
            }
            2 -> {
                when (stage) {
                    23 -> { npcl(FaceAnim.FRIENDLY, "Come on, let's have a staring contest!"); stage++ }
                    24 -> { playerl(FaceAnim.FRIENDLY, "You win!"); stage++ }
                    25 -> { npcl(FaceAnim.FRIENDLY, "Yay! I win again!"); stage++ }
                    26 -> { playerl(FaceAnim.FRIENDLY, "Oh, it's no contest alright."); stage = END_DIALOGUE }
                }
            }
            3 -> {
                when (stage) {
                    27 -> { npcl(FaceAnim.FRIENDLY, "You know, sometimes I don't think we're good friends."); stage++ }
