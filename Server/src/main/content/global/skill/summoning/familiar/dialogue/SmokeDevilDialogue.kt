...
class SmokeDevilDialogue : Dialogue {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        when (branch) {
            0 -> npcl(FaceAnim.CHILD_NORMAL, "When are you going to be done with that?")
            1 -> npcl(FaceAnim.CHILD_NORMAL, "Hey!")
            2 -> npcl(FaceAnim.CHILD_NORMAL, "Ah, this is the life!")
            3 -> npcl(FaceAnim.CHILD_NORMAL, "Why is it always so cold here?")
        }
        return true
    }
        ...
        when (branch) {
            0 -> {
                when (stage) {
                    ...
                    1 -> { npcl(FaceAnim.CHILD_NORMAL, "Good, because this place is too breezy."); stage++ }
                    2 -> { playerl(FaceAnim.HALF_ASKING, "What do you mean?"); stage++ }
                    3 -> { npcl(FaceAnim.CHILD_NORMAL, "I mean, it's tricky to keep hovering in this draft."); stage++ }
                    4 -> { playerl(FaceAnim.FRIENDLY, "Ok, we'll move around a little if you like."); stage++ }
                    5 -> { npcl(FaceAnim.CHILD_NORMAL, "Yes please!"); stage = END_DIALOGUE }
                }
            }
            1 -> {
                when (stage) {
                    0 -> { playerl(FaceAnim.HALF_ASKING, "Yes?"); stage++ }
                    1 -> { npcl(FaceAnim.CHILD_NORMAL, "Where are we going again?"); stage++ }
                    2 -> { playerl(FaceAnim.FRIENDLY, "Well, I have a lot of things to do today, so we might go a lot of places."); stage++ }
                    3 -> { npcl(FaceAnim.CHILD_NORMAL, "Are we there yet?"); stage++ }
                    4 -> { playerl(FaceAnim.FRIENDLY, "No, not yet."); stage++ }
                    5 -> { npcl(FaceAnim.CHILD_NORMAL, "How about now?"); stage++ }
                    6 -> { playerl(FaceAnim.FRIENDLY, "No."); stage++ }
                    7 -> { npcl(FaceAnim.CHILD_NORMAL, "Are we still not there?"); stage++ }
                    8 -> { playerl(FaceAnim.ANNOYED, "NO!"); stage++ }
                    9 -> { npcl(FaceAnim.CHILD_NORMAL, "Okay, just checking."); stage = END_DIALOGUE }
                }
            }
            2 -> {
                when (stage) {
                    0 -> { playerl(FaceAnim.HALF_ASKING, "Having a good time up there?"); stage++ }
                    1 -> { npcl(FaceAnim.CHILD_NORMAL, "Yeah! It's great to feel the wind in your tentacles."); stage++ }
                    2 -> { playerl(FaceAnim.FRIENDLY, "Sadly, I don't know what that feels like."); stage++ }
                    3 -> { npcl(FaceAnim.CHILD_NORMAL, "Why not?"); stage++ }
                    4 -> { playerl(FaceAnim.FRIENDLY, "No tentacles for a start."); stage++ }
                    5 -> { npcl(FaceAnim.CHILD_NORMAL, "Well, nobody's perfect."); stage = END_DIALOGUE }
                }
            }
            3 -> {
                when (stage) {
                    0 -> { playerl(FaceAnim.FRIENDLY, "I don't think it's that cold."); stage++ }
                    1 -> { npcl(FaceAnim.CHILD_NORMAL, "It is compared to back home."); stage++ }
                    2 -> { playerl(FaceAnim.FRIENDLY, "How hot is it where you are from?"); stage++ }
                    3 -> { npcl(FaceAnim.CHILD_NORMAL, "I can never remember. What is the vaporisation point of steel again?"); stage++ }
