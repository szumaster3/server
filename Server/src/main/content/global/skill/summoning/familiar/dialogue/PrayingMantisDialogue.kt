...
class PrayingMantisDialogue : Dialogue {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        when (branch) {
            ...
            4 -> npc(FaceAnim.CHILD_NORMAL, "Clatter chirrupchirp-", "(Today, grasshopper, we will-)")
        }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (branch) {
            0 -> when (stage) {
                ...
                1 -> { npc(FaceAnim.CHILD_NORMAL, "Clicker chirrpchirrup.", "(For catching the butterflies, grasshopper.)"); stage++ }
                2 -> { playerl(FaceAnim.FRIENDLY, "Oh, right! Well, if I use anything but the net I squash them."); stage++ }
                3 -> { npc(FaceAnim.CHILD_NORMAL, "Chirrupchirrup click!", "(Then, I could have them!)"); stage = END_DIALOGUE }
            }

            1 -> when (stage) {
                0 -> { playerl(FaceAnim.FRIENDLY, "Yes, almost every day."); stage++ }
                1 -> { npc(FaceAnim.CHILD_NORMAL, "Chirrupchirrup chirrup.", "('Almost' is not good enough.)"); stage++ }
                2 -> { playerl(FaceAnim.FRIENDLY, "Well, I'm trying as hard as I can."); stage++ }
                3 -> { npc(FaceAnim.CHILD_NORMAL, "Chirrup chitter chitter chirrup?", "(How do you expect to achieve enlightenment at this rate, grasshopper?)"); stage++ }
            ...
            2 -> when (stage) {
                ...
                1 -> { npc(FaceAnim.CHILD_NORMAL, "Clatter chitter click chitter...", "(Then we will wander about and punch monsters in the head...)"); stage++ }
                2 -> { playerl(FaceAnim.HALF_ASKING, "I could do in an enlightened way if you want?"); stage++ }
                3 -> { npc(FaceAnim.CHILD_NORMAL, "Chirrupchitter!", "(That will do!)"); stage = END_DIALOGUE }
            }

            3 -> when (stage) {
                0 -> { playerl(FaceAnim.HALF_ASKING, "Is there any point to that saying?"); stage++ }
                1 -> { npc(FaceAnim.CHILD_NORMAL, "Clatter chirrupchirrup chirp.", "(I find that a happy mantis is its own point.)"); stage = END_DIALOGUE }
            }

            4 -> when (stage) {
                0 -> { playerl(FaceAnim.FRIENDLY, "You know, I'd rather you call me something other than grasshopper."); stage++ }
                1 -> { npc(FaceAnim.CHILD_NORMAL, "Clitterchirp?", "(Is there a reason for this?)"); stage++ }
                2 -> { playerl(FaceAnim.FRIENDLY, "You drool when you say it."); stage++ }
                3 -> { npc(FaceAnim.CHILD_NORMAL, "Clickclatter! Chirrup chirpchirp click chitter...", "(I do not! Why would I drool when I call you a juicy...)"); stage++ }
                4 -> { npc(FaceAnim.CHILD_NORMAL, "...clickclick chitter clickchitter click...", "(...succulent, nourishing, crunchy...)"); stage++ }
                5 -> { npc(FaceAnim.CHILD_NORMAL, "*Drooool*"); stage++ }
