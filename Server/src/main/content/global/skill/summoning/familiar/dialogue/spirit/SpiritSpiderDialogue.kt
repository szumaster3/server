...
class SpiritSpiderDialogue : Dialogue {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        when (branch) {
            0 -> npcl(FaceAnim.CHILD_NORMAL, "Where are we going?")
            1 -> npcl(FaceAnim.CHILD_NORMAL, "Who is that?")
            2 -> npcl(FaceAnim.CHILD_NORMAL, "What are you doing?")
            3 -> npcl(FaceAnim.CHILD_NORMAL, "Sigh...")
            4 -> npcl(FaceAnim.CHILD_NORMAL, "So, do I get any of those flies?")
        }
        ...
        when (branch) {
            0 -> {
                when (stage) {
                    ...
                    1 -> { npcl(FaceAnim.CHILD_NORMAL, "Fine, don't tell me..."); stage++ }
                    2 -> { playerl(FaceAnim.FRIENDLY, "Oh, okay, well, we are going..."); stage++ }
                    3 -> { npcl(FaceAnim.CHILD_NORMAL, "Don't want to know now."); stage++ }
            ...
            1 -> {
                when (stage) {
                    ...
                    1 -> { npcl(FaceAnim.CHILD_NORMAL, "The two-legs over there."); stage++ }
                    2 -> { playerl(FaceAnim.FRIENDLY, "I can't see who you mean..."); stage++ }
                    3 -> { npcl(FaceAnim.CHILD_NORMAL, "Never mind..."); stage++ }
                    4 -> { playerl(FaceAnim.FRIENDLY, "Can you describe them a little better..."); stage++ }
                    5 -> { npcl(FaceAnim.CHILD_NORMAL, "It doesn't matter now."); stage++ }
            ...
            2 -> {
                when (stage) {
                    ...
                    1 -> { npcl(FaceAnim.CHILD_NORMAL, "I see, you don't think I'm smart enough to understand..."); stage++ }
                    2 -> { playerl(FaceAnim.FRIENDLY, "That's not it at all! Look, I was..."); stage++ }
                    3 -> { npcl(FaceAnim.CHILD_NORMAL, "Don't wanna know now."); stage++ }
                    4 -> { playerl(FaceAnim.FRIENDLY, "Siiiigh...spiders."); stage = END_DIALOGUE }
                }
            }
            3 -> {
                when (stage) {
                    ...
                    1 -> { npcl(FaceAnim.CHILD_NORMAL, "Nothing really."); stage++ }
            ...
            4 -> {
                when (stage) {
                    ...
                    1 -> { npcl(FaceAnim.CHILD_NORMAL, "I see..."); stage++ }
                    2 -> { playerl(FaceAnim.FRIENDLY, "Look, you can have some if you want."); stage++ }
                    3 -> { npcl(FaceAnim.CHILD_NORMAL, "Oh, don't do me any favours."); stage++ }
                    4 -> { playerl(FaceAnim.FRIENDLY, "Look, here, have some!"); stage++ }
                    5 -> { npcl(FaceAnim.CHILD_NORMAL, "Don't want them now."); stage++ }
                    6 -> { playerl(FaceAnim.FRIENDLY, "Siiiigh...spiders."); stage = END_DIALOGUE }
                }
            }
        }
