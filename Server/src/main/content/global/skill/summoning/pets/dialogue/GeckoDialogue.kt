...
class GeckoDialogue(player: Player? = null) : Dialogue(player) {
    private val babyGecko = intArrayOf(NPCs.BABY_GECKO_6915, NPCs.BABY_GECKO_7277, NPCs.BABY_GECKO_7278, NPCs.BABY_GECKO_7279, NPCs.BABY_GECKO_7280)
    private val adultGecko = intArrayOf(NPCs.GECKO_6916, NPCs.GECKO_7281, NPCs.GECKO_7282, NPCs.GECKO_7283, NPCs.GECKO_7284)

    override fun open(vararg args: Any?): Boolean {
        npc = args[0] as NPC
        when ((0..4).random()) {
            0 -> npc(FaceAnim.CHILD_NORMAL, "Moop, Gecko presko!", "(You're nice to me!)").also { stage = END_DIALOGUE }
            1 -> npc(FaceAnim.CHILD_NORMAL, "Gronk, Gecko gronk?", "(When can we go back to the jungle?)").also { stage = 0 }
            ...
            else -> if (inBorders(player, getRegionBorders(12598)) || inZone(player, "Desert Zone")) {
    ...
    override fun handle(componentID: Int, buttonID: Int): Boolean {
        when (stage) {
            ...
            2 -> npc(FaceAnim.CHILD_NORMAL, "Gecko! Gronk gronk.", "(It's hard to keep up with you!)").also { stage = END_DIALOGUE }
