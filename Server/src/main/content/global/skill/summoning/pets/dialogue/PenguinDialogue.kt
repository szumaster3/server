...
class PenguinDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        when ((0..3).random()) {
            ...
            else -> if (inZone(player, "snow") || inBorders(player, getRegionBorders(11830)) || inBorders(player, getRegionBorders(11318))
            ) {
    ...
    override fun handle(componentID: Int, buttonID: Int): Boolean {
        when (stage) {
            ...
            5 -> playerl(FaceAnim.FRIENDLY, "Hmm, I dunno, the penguins there are a bit shifty. You shouldn't mix with them.").also { stage = END_DIALOGUE }
            6 -> playerl(FaceAnim.FRIENDLY, "Well, just give me a little while longer and maybe we'll go find some ice for you.").also { stage++ }
            7 -> npc(FaceAnim.CHILD_NORMAL, "Awk! Awkawk awk awk. (Yay! Don't be too long.)").also { stage = END_DIALOGUE }
            8 -> playerl(FaceAnim.FRIENDLY, "Why? Is there some place else where you'd rather be?").also { stage++ }
            9 -> npc(FaceAnim.CHILD_NORMAL, "Awk awk awk!", "(For sure. Somewhere colder! Like the Motherland...)").also { stage = END_DIALOGUE }
