package content.global.skill.slayer.dungeon.waterbirth_dungeon
...
class SpinolypNPC(id: Int = 0, location: Location? = null) : AbstractNPC(id, location) {
    ...
    private val DESPAWN_DISTANCE = 6
    ...
    override fun tick() {
        val players = RegionManager
            .getLocalPlayers(location, DESPAWN_DISTANCE)
    ...
    override fun sendImpact(state: BattleState) {
        state.estimatedHit = when {
            state.estimatedHit == 0 && RandomFunction.random(15) < 2 ->
                RandomFunction.random(1, 11)
    ...
    override fun getIds(): IntArray =
        intArrayOf(NPCs.SPINOLYP_2894, NPCs.SPINOLYP_2896)

    companion object Combat {
        private val SPELL: CombatSpell? =
            SpellBookManager.SpellBook.MODERN.getSpell(14) as? CombatSpell

        val SWING_HANDLER = object : MultiSwingHandler(
            ...
            override fun canSwing(entity: Entity, victim: Entity): InteractionType {
                ...
                return if (type == InteractionType.MOVE_INTERACT)
                    InteractionType.NO_INTERACT
                else type ?: InteractionType.NO_INTERACT
            }
            ...
            override fun impact(entity: Entity?, victim: Entity?, state: BattleState?) {
                if (state?.style == CombatStyle.MAGIC && (state.estimatedHit ?: 0) > 0) {
                    ...
                    RandomFunction.random(4) == 0 &&
                    entity != null &&
                    victim != null
                ) {
                    applyPoison(victim, entity, 68)
                }
            }
        }
    }
