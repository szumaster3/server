...
class LeprechaunNote : InteractionListener {
    ...
    private val tooLeprechaun = intArrayOf(
        NPCs.TOOL_LEPRECHAUN_3021, NPCs.GOTH_LEPRECHAUN_8000, NPCs.TOOL_LEPRECHAUN_4965, NPCs.TECLYN_2861
    )
    ...
    override fun defineListeners() {
        onUseWith(IntType.NPC, cropId, *tooLeprechaun) { player, used, with ->
            ...
            if (usedItem.noteChange != usedItem.id) {
                ...
                sendItemDialogue(
                    player, usedItem.id, "The leprechaun exchanges your ${usedItem.name.lowercase()} for a banknote."
                )
            } else {
                sendNPCDialogue(
                    player, npc.id, "Nay, I've got no banknotes to exchange for that item.", facialExpression
                )
            }
