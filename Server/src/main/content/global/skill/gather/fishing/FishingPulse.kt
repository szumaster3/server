...
class FishingPulse(player: Player?, npc: NPC, private val option: FishingOption?) : SkillPulse<NPC?>(player, npc) {
    ...
    override fun checkRequirements(): Boolean {
        ...
        if (!inInventory(player, option.tool) && !hasBarbTail()) {
            ...
            msg += if (option.optionName in arrayOf("lure", "bait")
            ) "${option.optionName} these fish." else "catch these fish."
        ...
        if (freeSlots(player) == 0) {
            if (option.fishId.contains(Fish.LOBSTER)) {
    ...
    override fun animate() {
        if (isBareHanded(player)) {
            ...
            player.animate(Animation(option!!.animationId))
        }
    }
    ...
    override fun reward(): Boolean {
        ...
        if (success()) {
            if (player.inventory.hasSpaceFor(Item(fish!!.id)) && option!!.removeBait(player)) {
                ...
                player.skills.addExperience(Skills.FISHING, fish!!.xp, true)
                message(2)
            }
        }
    ...
    private fun isBareHanded(p: Player): Boolean {
        if (option == FishingOption.Harpoon || option == FishingOption.SharkHarpoon) {
