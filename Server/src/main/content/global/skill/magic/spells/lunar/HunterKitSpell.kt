    ...
    override fun defineListeners() {
        onCast(LunarSpells.HUNTER_KIT, NONE) { player, _ ->
            if (freeSlots(player) == 0) {
                sendMessage(player, "You need at least one free inventory space to cast this spell.").also { return@onCast }
            }
            ...
            if (addItem(player, Items.HUNTER_KIT_11159)) {
                visualizeSpell(player, Animations.LUNAR_HUNTER_KIT_6303, Graphics.MAKE_HUNTER_KIT_1074, soundID = Sounds.LUNAR_HUNTER_KIT_3615)
                addXP(player, 70.0)
                setDelay(player, 2)
            }
        }
