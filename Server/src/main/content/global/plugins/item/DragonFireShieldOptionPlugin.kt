...
class DragonFireShieldOptionPlugin : InteractionListener {
    override fun defineListeners() {
        ...
        on(dragonFireShields, IntType.ITEM, "operate") { player, node ->
            ...
            val shieldItem = node.asItem()
            val notCharged = shieldItem.id == Items.DRAGONFIRE_SHIELD_11284 || shieldItem.charge < 20
            if (notCharged) {
            ...
            val attack = SwitchAttack(
                ...
                Projectile.create(
                    player, null, shared.consts.Graphics.DRAGONFIRE_SHIELD_PROJECTILE_1166,
                    36, 36, 80, 70, 0, 11
                ),
            )
            ...
            val handler: DragonfireSwingHandler = object : DragonfireSwingHandler(false, 25, attack, true) {
                override fun swing(entity: Entity?, victim: Entity?, state: BattleState?): Int {
                    if (entity is Player) {
                        ...
                        if (shield.charge < 20 && shieldItem.slot == EquipmentContainer.SLOT_SHIELD) {
                            val oldItem = shield
                            val slot = oldItem.slot
                            val newItem = Item(Items.DRAGONFIRE_SHIELD_11284)
        ...
        on(dragonFireShields, IntType.ITEM, "inspect") { player, node ->
            val shield = node.asItem()
            if (shield.id == Items.DRAGONFIRE_SHIELD_11284) {
                sendMessage(player, "The shield has no charges.")
                return@on true
            }
            sendMessage(player, "The shield has ${shield.charge / 20} charges.")
            return@on true
        }
    ...
    companion object {
        ...
        private const val dfsEmptyAnim = Animations.SHIELD_DRAGONFIRE_6700
        private const val dfsEmptyGfx = shared.consts.Graphics.DFS_FUSE_DRAGONKIN_KEY_CHARGE_1160
