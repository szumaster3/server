package content.global.plugins.iface.appearance
...
class ThessaliaInterface : ComponentPlugin() {
            ...
            181, 180, 297 -> pay(player)
                ...
                THESSALIA_MALE_COMPONENT -> {
                    ...
                    if (maleColorButtonRange.contains(button)) {
                        updateColor(player, button, true, player.getAttribute("thes-type", ColorType.TORSO),)
                    }
                }
                ...
                THESSALIA_FEMALE_COMPONENT -> {
                    ...
                    if (femaleColorButtonRange.contains(button)) {
                        updateColor(player, button, false, player.getAttribute("thes-type", ColorType.TORSO))
                    }
                }
            }
        }
    ...
    private fun updateTop(player: Player, button: Int, male: Boolean) {
    ...
    private fun updateLegs(player: Player, button: Int, male: Boolean) {
    ...
    private fun updateArms(player: Player, button: Int, male: Boolean) {
    ...
    private fun updateColor(player: Player, button: Int, male: Boolean, type: ColorType) {
        val subtract = if (male) maleColorButtonRange.first else femaleColorButtonRange.first
        when (type) {
            ColorType.ARMS -> player.appearance.torso.changeColor(torsoColors[button - subtract])
