...
class Adventurer(val style: CombatStyle) : Script() {
    ...
    override fun toString(): String {
        return "${bot.username} is an Adventurer bot " + "at ${bot.location}! " + "State: $state - " + "City: $city - " + "Ticks: $ticks - " + "Freshspawn: $freshspawn - " + "Sold: $sold - " + "Counter: $counter"
    }
...
//                    log(this::class.java, Log.FINE, "${bot.username} has moved on to a different city at $ticks ticks and $counter counter.")
    ...
    fun dialogue() {
        ...
        if (localPlayers.isNotEmpty()) {
            val localPlayer = localPlayers.filter { it.name != bot.name }.randomOrNull()
            if (localPlayer != null) {
    ...
    enum class State {
        START, ADVENTURE, FIND_BANK, FIND_CITY, IDLE_GE, GE, TELEPORTING, LOOT, LOOT_DELAY, FIND_GE
    }
    ...
    companion object {
        ...
        private fun JsonObject.getLines(category: String): JsonArray {
            return this.getAsJsonArray(category)
        }
